...
try {
  ...
  const vr2 = await verifyRedStatus(
    'node -e \"console.error(\\\"AssertionError: x\\\"); process.exit(1)\"',
    { timeoutMs: 5000 }
  );
  ...
  const vr3 = await verifyRedStatus(
    'node -e \"console.error(\\\"ImportError: foo\\\"); process.exit(1)\"',
    { timeoutMs: 5000 }
  );
