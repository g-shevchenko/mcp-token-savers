...
SERVICE_DIR="$ROOT_DIR/services/tdd-gate-mcp"

cd "$ROOT_DIR"

export TDD_GATE_CACHE_DIR="${TDD_GATE_CACHE_DIR:-$HOME/.hwai/tdd-gate-mcp}"
...
if [[ -z "$NODE_BIN" ]]; then
  echo "node executable not found; set NODE_BIN for tdd-gate-mcp local stdio" >&2
  exit 127
fi
...
if [[ ! -f "$SERVICE_DIR/dist/index.js" ]] || \
  ...
  if [[ -z "$NPM_BIN" ]]; then
    echo "npm executable not found; run npm run build or set NPM_BIN for tdd-gate-mcp local stdio" >&2
    exit 127
  fi
