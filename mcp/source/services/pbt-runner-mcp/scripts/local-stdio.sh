...
SERVICE_DIR="$ROOT_DIR/services/pbt-runner-mcp"

cd "$ROOT_DIR"

export PBT_RUNNER_CACHE_DIR="${PBT_RUNNER_CACHE_DIR:-$HOME/.hwai/pbt-runner-mcp}"
...
if [[ -z "$NODE_BIN" ]]; then
  echo "node executable not found; set NODE_BIN for pbt-runner-mcp local stdio" >&2
  exit 127
fi
...
if [[ ! -f "$SERVICE_DIR/dist/index.js" ]] || \
  ...
  if [[ -z "$NPM_BIN" ]]; then
    echo "npm executable not found; run npm run build or set NPM_BIN for pbt-runner-mcp local stdio" >&2
    exit 127
  fi
