...
import { DOCS_HYGIENE_MEASUREMENT_SCHEMA_VERSION, DocsHygieneConfig } from "./config.js";
...
export async function buildMeasurementReport(config: DocsHygieneConfig, args: MeasurementArgs = {}) {
  ...
  const quality = {
    ...
    broken_anchors_count: okRows.reduce((sum, row) => sum + num(row.output?.broken_anchors_count), 0),
    broken_links_count: okRows.reduce((sum, row) => sum + num(row.output?.broken_links_count), 0),
    doc_count: okRows.reduce((sum, row) => sum + num(row.output?.doc_count), 0),
    doc_lines: okRows.reduce((sum, row) => sum + num(row.output?.doc_lines), 0),
    duplicate_section_groups: okRows.reduce((sum, row) => sum + num(row.output?.duplicate_section_groups), 0),
    ...
    orphan_docs_count: okRows.reduce((sum, row) => sum + num(row.output?.orphan_docs_count), 0),
    plan_items_count: okRows.reduce((sum, row) => sum + num(row.output?.plan_items_count), 0),
    ssot_conflicts_count: okRows.reduce((sum, row) => sum + num(row.output?.ssot_conflicts_count), 0),
    stale_references_count: okRows.reduce((sum, row) => sum + num(row.output?.stale_references_count), 0),
  };

  return {
    schema_version: DOCS_HYGIENE_MEASUREMENT_SCHEMA_VERSION,
    date,
    time_basis: "UTC",
    window: {
      since_iso: sinceIso,
