...
import { GoldenDatasetConfig } from "./config.js";
import { redactSecrets } from "./text-utils.js";

export function artifactFileName(raw: string): string {
  const cleaned = raw.replace(/^golden-dataset:\/\/local\/artifacts\//, "");
...
export async function readArtifact(config: GoldenDatasetConfig, fileName: string): Promise<Buffer | null> {
  const safeName = path.basename(fileName);
  const fullPath = path.join(config.artifactDir, safeName);
  try {
