...
if [[ -z "$NODE_BIN" ]]; then
  echo "node executable not found; set NODE_BIN for crawl4ai-mcp local stdio" >&2
  exit 127
fi
if [[ ! -f "$SERVICE_DIR/dist/index.js" ]] || \
  ...
  if [[ -z "$NPM_BIN" ]]; then
    echo "npm executable not found; run npm run build or set NPM_BIN for crawl4ai-mcp local stdio" >&2
    exit 127
  fi
