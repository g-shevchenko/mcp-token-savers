...
import { PlaywrightTraceConfig } from "./config.js";
import { redactSecrets } from "./text-utils.js";

export function artifactFileName(raw: string): string {
  const cleaned = raw.replace(/^playwright-trace:\/\/local\/artifacts\//, "");
  return path.basename(cleaned);
}

export async function writeArtifact(
  config: PlaywrightTraceConfig,
  fileName: string,
...
export async function readArtifact(config: PlaywrightTraceConfig, fileName: string): Promise<Buffer | null> {
  const safeName = path.basename(fileName);
  const fullPath = path.join(config.artifactDir, safeName);
  try {
