...
import { PLAYWRIGHT_TRACE_MEASUREMENT_SCHEMA_VERSION, PlaywrightTraceConfig } from "./config.js";
...
export async function buildMeasurementReport(config: PlaywrightTraceConfig, options: MeasurementOptions = {}) {
  ...
  const consoleErrors = okRequests.reduce((sum, line) => sum + number(line.output?.console_errors), 0);
  const networkFailures = okRequests.reduce((sum, line) => sum + number(line.output?.network_failures), 0);
  const failuresFound = okRequests.filter((line) => line.output?.failure_found === true).length;
  const failureWindows = okRequests.filter((line) => line.output?.failure_window_present === true).length;
  const failureWindowConsoleErrors = okRequests.reduce(
    (sum, line) => sum + number(line.output?.failure_window_console_errors),
    0,
  );
  const failureWindowNetworkFailures = okRequests.reduce(
    (sum, line) => sum + number(line.output?.failure_window_network_failures),
    0,
  );
  const failureWindowSlowRequests = okRequests.reduce(
    (sum, line) => sum + number(line.output?.failure_window_slow_requests),
    0,
  );
  const screenshotsPrepared = okRequests.reduce((sum, line) => sum + number(line.output?.screenshots_prepared), 0);
  const p95LatencyMs = percentile(latencies, 95);
  const estimatedUsdSaved = round((savedTokens / 1_000_000) * config.measurementUsdPer1MTokens, 4);

  return {
    schema_version: PLAYWRIGHT_TRACE_MEASUREMENT_SCHEMA_VERSION,
    date,
    time_basis: "UTC",
    window: {
    ...
    quality: {
      ...
      failure_window_console_errors: failureWindowConsoleErrors,
      failure_window_network_failures: failureWindowNetworkFailures,
      failure_window_slow_requests: failureWindowSlowRequests,
      screenshots_prepared: screenshotsPrepared,
    },
    token_savings: {
    ...
    pantheon_export: {
      ...
      data_policy: {
        aggregate_only: true,
        includes_raw_trace: false,
        includes_raw_console: false,
        includes_raw_network: false,
